...
uint256 hashGenesisBlock("0x00000063327f52e47458d984bc619e72cba8ea7f15d0fb1b08dd3e08e94f2150");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // StandardCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "StandardCoin Signed Message:\n";
...
{
    // StandardCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // StandardCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) { 
		...
		const char* pszTimestamp = "standard genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1492707952;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1747805;
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // StandardCoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("StandardCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("StandardCoinMiner : ProcessBlock, block not accepted");
    }
